...
for it=1:ts                     %iterate for ts time steps
    ...
    %*** 6. PLOT RESULTS ***
	if (mod(it,1) == 0 || it==ts)              %plot only every 25 time steps
        ...
        title(sprintf('Density %i, np %i', it, np));
        hold off;
        caxis([0 8e11])
        colormap('jet');

        save2gif(it,'density.gif',0.1);
		figure(2);                              %potential
		contourf(phi');
        hold on;
        contour(object',[1 1],'-k','LineWidth',2);
        hold off;
		title(sprintf('Potential %i, np %i', it, np));
        colorbar;
        caxis([-5, 1])
        colormap('jet');
        save2gif(it,'Potential.gif',0.1);
